...
func TestCalculateRiskScore(t *testing.T) {
	tests := []struct {
		...
		{"Low Risk", 2, 0, false, 1, 5},
		{"Medium Risk", 10, 1, false, 15, 20},
		{"High Risk", 20, 2, true, 35, 45},
		{"Max Risk", 200, 10, true, 100, 100},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
score := CalculateRiskScore(tt.filesChanged, tt.hotspotFiles, tt.isCrossDir)
